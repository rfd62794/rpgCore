...
class BreedingScene(Scene):
    ...
    def render(self, surface: pygame.Surface):
        ...
        if self.state in [BreedingState.SELECT_A, BreedingState.SELECT_B]:
            ...
            Label("POTENTIAL LEGACY", (400, 170), font_size=18, color=(200, 220, 255)).render(surface)
            ...
            for label, base_val, p_pref in [("HP", self.offspring_genome.base_hp, max(self.parent_a.genome.base_hp, self.parent_b.genome.base_hp)), 
                ...
                Label(msg, (400, y_off), font_size=16, color=(200, 200, 220)).render(surface)
                
                # Improvement indicator (Simplified comparison)
                if base_val > p_pref:
                     Label("(Up)", (480, y_off), font_size=12, color=(100, 255, 100)).render(surface)
            ...
            Label(f"Mutation Risk: {mut_label}", (400, 290), font_size=14, color=(180, 140, 200)).render(surface)
            Label(f"Culture: {self.offspring_genome.cultural_base.value.upper()}", (400, 310), font_size=14, color=(200, 200, 200)).render(surface)
            
            # Improvement note
            Label("Generational Gain: Diminishing", (400, 340), font_size=14, color=(100, 220, 100)).render(surface)
            Label(f"Target Gen: {self.offspring_genome.generation}", (400, 360), font_size=14, color=(150, 150, 255)).render(surface)

            if self.parent_a.is_elder or self.parent_b.is_elder:
                Label("ELDER BONUS: LEVEL 2 START", (400, 380), font_size=14, color=(255, 215, 0)).render(surface)
            ...
            Label("Welcome Home!", (400, 400), font_size=32, color=(100, 255, 100)).render(surface)
