...
def draw_number_face(screen, cx, cy, r, value, colors, fonts,
    ...
    # Calculate bounding box for surf creation to avoid fullscreen surface
    if not pts:
        return
    ...
    local_pts = [(p[0] - min_x + 5, p[1] - min_y + 5) for p in pts]

    body_col  = (*colors["body"], alpha)
    edge_col  = (*colors["edge"], alpha)

    if len(local_pts) >= 3:
        pygame.draw.polygon(body_surf, body_col, local_pts)
        pygame.draw.polygon(body_surf, edge_col, local_pts, 2)
        ...
        for px, py in half:
            ...
            hi_pts.append((int(cx + rx) - min_x + 5, int(cy + ry) - min_y + 5))
        if len(hi_pts) >= 3:
            pygame.draw.polygon(body_surf, (255, 255, 255, min(alpha, 28)), hi_pts)

    screen.blit(body_surf, (min_x - 5, min_y - 5))
    ...
    if sides in (4, 6) and value <= 6:
        ...
        # If font missing, fallback to whatever we have (or skip rendering textually)
        f_obj = fonts.get(font_key)
        if not f_obj:
            return

        surf  = f_obj.render(label, True, num_col)
...
class Die:
    ...
    def draw(self, screen, fonts):
        colors    = DIE_COLORS.get(self.sides, DIE_COLORS[6])
        r         = self.radius
        ...
        f_label = fonts.get("label")
        if not f_label:
            return

        if self.state == "settled" and self.glow > 0.25:
            if is_crit:
                lbl = f_label.render("CRIT!", True, GOLD)
                screen.blit(lbl, lbl.get_rect(center=(cx, cy + r + 16)))
            elif is_fumble:
                lbl = f_label.render("FUMBLE", True, (255, 75, 75))
                screen.blit(lbl, lbl.get_rect(center=(cx, cy + r + 16)))

        lc   = colors["edge"] if self.hovered else (88, 88, 108)
        lsurf = f_label.render(self.label, True, lc)
